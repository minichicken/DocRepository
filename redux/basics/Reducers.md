...
>##### Note on Relationships 관계성에 초점을 두자.
...
더 복잡한 어플리케이션에서, 당시은 각각 다 다른 개체들로 되기를 원한다. 우리는 당신의 상태를 가능한 표준화하고, 어떤 끼어맞추건 없이. 모든 개체들을 객체로 Id와 Key를 가지고 저장하고, 다른 개체들과 리스트로 부터 참조하기 위해서 Id를 사용해라. 데이터베이스에 어플리케이션의 상태를 생각해라. 이러한 접근법은  normalizr의 문서에 상세하게 기술되어 있다. 예를들면  상태안에서 `todosById: { id -> todo }`그리고 `todos: array<id>` 유지하는것은 실제 어플리케잇 녀에 아주 좋은 생각이지만, 우리는 간단한 예를 들어보자.

## Handling Actions(액션들은 핸들링하기)
